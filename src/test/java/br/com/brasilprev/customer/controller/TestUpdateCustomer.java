...
public class TestUpdateCustomer {
	...
	public void testAddEmployee() throws Exception {
		...
		when(customerService.update(1, customerStub)).thenReturn(customerStub);

		Gson GSON = new Gson();

		mockMvc.perform(MockMvcRequestBuilders.put("/brasilprev/v1/customer/1")
				.contentType(MediaType.APPLICATION_JSON)
				.content(GSON.toJson(customerStub))
				.accept(MediaType.APPLICATION_JSON))
				.andExpect(status().isOk());

		verify(customerService).update(1, customerStub);
